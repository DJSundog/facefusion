...
namespace FaceFusion.ViewModels
{
    public class FusionManager : ViewModelBase, IDisposable
    {
        ...
        DepthImageFormat _currentDepthFormat = DepthImageFormat.Undefined;
        ...
        public void ProcessFusionFrame(KinectFrameWorkItem workItem)
        {
            ...
            if (format != _currentDepthFormat)
            {
                _currentDepthFormat = format;

                _fusionWorkItemPool.Format = format;
        ...
        private void FusionUpdateUI(object state)
        {
            ...
            var depthSize = FormatHelper.GetDepthSize(_currentDepthFormat);
        ...
        private void ProcessFusionFrameBackground(FusionWorkItem workItem)
        {
            ...
            try
            {
                ...
                {
                    trackingSucceeded = TrackIntegrate(depthPixels, workItem.Format);
        ...
        private void SaveBinarySTLMesh(Mesh mesh, BinaryWriter writer, bool flipYZ = true)
        {
            ...
            {
                this.FusionStatusMessage = "Error saving mesh: Mesh is invalid.";
            }
        ...
        /// <param name="flipYZ">Flag to determine whether the Y and Z values are flipped on save</param>
        private void SaveAsciiObjMesh(Mesh mesh, StreamWriter writer, bool flipYZ = true)
        {
            var vertices = mesh.GetVertices();
            var normals = mesh.GetNormals();
            var indices = mesh.GetTriangleIndexes();
